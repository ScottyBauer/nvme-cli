...
int nvme_get_log(int fd, __u32 nsid, __u8 log_id, __u8 lsp,
...
int _nvme_get_log(int fd, __u32 nsid, __u8 log_id, __u8 lsp, __u32 LPO,
		 __u32 data_len, void *data)
{
	struct nvme_admin_cmd cmd = {
		.opcode		= nvme_admin_get_log_page,
...
int nvme_get_telemetry_log(int fd, void *lp,
			   int generate_report,
			   size_t log_page_size, __u32 offset)
{

	if (generate_report)
		return _nvme_get_log(fd, NVME_NSID_ALL, NVME_LOG_TELEMETRY_HOST,
...
{
	return nvme_get_log(fd, NVME_NSID_ALL, NVME_LOG_FW_SLOT, NVME_NO_LOG_LSP,
			    sizeof(*fw_log), fw_log);
}

int nvme_error_log(int fd, int entries, struct nvme_error_log_page *err_log)
{
	return nvme_get_log(fd, 0, NVME_LOG_ERROR, NVME_NO_LOG_LSP,
			    entries * sizeof(*err_log), err_log);
}

int nvme_smart_log(int fd, __u32 nsid, struct nvme_smart_log *smart_log)
{
	return nvme_get_log(fd, nsid, NVME_LOG_SMART, NVME_NO_LOG_LSP,
			    sizeof(*smart_log), smart_log);
}

int nvme_discovery_log(int fd, struct nvmf_disc_rsp_page_hdr *log, __u32 size)
{
	return nvme_get_log(fd, 0, NVME_LOG_DISC, NVME_NO_LOG_LSP, size, log);
}

int nvme_sanitize_log(int fd, struct nvme_sanitize_log_page *sanitize_log)
{
	return nvme_get_log(fd, 0, NVME_LOG_SANITIZE, NVME_NO_LOG_LSP,
